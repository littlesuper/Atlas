...
describe('release observation batch 405 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-03T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch405 EvalError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new EvalError(`status-batch405-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 196,
        samples: [{ checkedAt, status, failedChecks: ['batch405'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-03T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `splice-batch405-${index}`,
  ] as const))(
    'generated batch405 failedChecks splice after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['a', replacementFailure, 'b'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.splice(1, 1, 'x');

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['a', 'x', 'b']);
    },
  );
});

describe('release observation batch 406 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-04T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch406 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([], `status-batch406-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 197,
        samples: [{ checkedAt, status, failedChecks: ['batch406'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-04T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch406-${index}`,
  ] as const))(
    'generated batch406 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: [replacementFailure] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('prepended');
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['prepended', replacementFailure]);
    },
  );
});

describe('release observation batch 407 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-05T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch407 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch407-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 198,
        samples: [{ checkedAt, status, failedChecks: ['batch407'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-05T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch407-${index}`,
  ] as const))(
    'generated batch407 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: [replacementFailure, 'end'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
...
describe('release observation batch 408 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-06T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch408 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch408-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 199,
        samples: [{ checkedAt, status, failedChecks: ['batch408'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-06T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch408-${index}`,
  ] as const))(
    'generated batch408 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['head', replacementFailure, 'tail'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.reverse();
...
describe('release observation batch 409 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-07T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch409 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch409-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 200,
        samples: [{ checkedAt, status, failedChecks: ['batch409'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-07T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch409-${index}`,
  ] as const))(
    'generated batch409 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['z', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
...
describe('release observation batch 410 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-08T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch410 URIError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new URIError(`status-batch410-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 201,
        samples: [{ checkedAt, status, failedChecks: ['batch410'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-08T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `copyWithin-batch410-${index}`,
  ] as const))(
    'generated batch410 failedChecks copyWithin after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['x', replacementFailure, 'y'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.copyWithin(0, 2);
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['y', replacementFailure, 'y']);
    },
  );
});
...
describe('release observation batch 412 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-10T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch412 EvalError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new EvalError(`status-batch412-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 203,
        samples: [{ checkedAt, status, failedChecks: ['batch412'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-10T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `splice-batch412-${index}`,
  ] as const))(
    'generated batch412 failedChecks splice after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['m', replacementFailure, 'n'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.splice(0, 1, 'replaced');

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['replaced', replacementFailure, 'n']);
    },
  );
});

describe('release observation batch 413 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-11T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch413 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([], `status-batch413-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 204,
        samples: [{ checkedAt, status, failedChecks: ['batch413'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-11T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch413-${index}`,
  ] as const))(
    'generated batch413 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: [replacementFailure] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('first');
...
describe('release observation batch 414 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-12T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch414 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch414-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 205,
        samples: [{ checkedAt, status, failedChecks: ['batch414'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-12T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch414-${index}`,
  ] as const))(
    'generated batch414 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: [replacementFailure, 'removeme'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
...
describe('release observation batch 415 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-13T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch415 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch415-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 206,
        samples: [{ checkedAt, status, failedChecks: ['batch415'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-13T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch415-${index}`,
  ] as const))(
    'generated batch415 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['a', replacementFailure, 'c']);
    },
  );
});

describe('release observation batch 416 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-14T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch416 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch416-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 207,
        samples: [{ checkedAt, status, failedChecks: ['batch416'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-14T02:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch416-${index}`,
  ] as const))(
    'generated batch416 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['a', replacementFailure, 'c'].sort());
    },
  );
});

describe('release observation batch 417 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-15T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch417 URIError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new URIError(`status-batch417-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 208,
        samples: [{ checkedAt, status, failedChecks: ['batch417'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-15T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `copyWithin-batch417-${index}`,
  ] as const))(
    'generated batch417 failedChecks copyWithin after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
...
describe('release observation batch 418 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-16T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch418 ReferenceError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new ReferenceError(`status-batch418-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 209,
        samples: [{ checkedAt, status, failedChecks: ['batch418'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-16T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `fill-batch418-${index}`,
  ] as const))(
    'generated batch418 failedChecks fill after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.fill('x', 1, 2);

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['c', 'x', 'a']);
    },
  );
});
...
describe('release observation batch 420 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-18T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch420 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([`err-${index}`], `status-batch420-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 211,
        samples: [{ checkedAt, status, failedChecks: ['batch420'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-18T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch420-${index}`,
  ] as const))(
    'generated batch420 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('z');

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['z', 'c', replacementFailure, 'a']);
    },
  );
});

describe('release observation batch 421 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-19T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch421 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch421-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 212,
        samples: [{ checkedAt, status, failedChecks: ['batch421'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-19T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch421-${index}`,
  ] as const))(
    'generated batch421 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['c', replacementFailure]);
    },
  );
});

describe('release observation batch 422 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-20T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch422 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch422-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 213,
        samples: [{ checkedAt, status, failedChecks: ['batch422'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-20T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch422-${index}`,
  ] as const))(
    'generated batch422 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.reverse();
...
describe('release observation batch 423 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-21T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch423 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch423-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 214,
        samples: [{ checkedAt, status, failedChecks: ['batch423'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-21T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch423-${index}`,
  ] as const))(
    'generated batch423 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
...
describe('release observation batch 425 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-23T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch425 ReferenceError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new ReferenceError(`status-batch425-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 216,
        samples: [{ checkedAt, status, failedChecks: ['batch425'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-23T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `fill-batch425-${index}`,
  ] as const))(
    'generated batch425 failedChecks fill after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.fill('x', 1, 2);
...
describe('release observation batch 426 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-24T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch426 EvalError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new EvalError(`status-batch426-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 217,
        samples: [{ checkedAt, status, failedChecks: ['batch426'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-24T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `splice-batch426-${index}`,
  ] as const))(
    'generated batch426 failedChecks splice after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.splice(1, 1, 'y');
...
describe('release observation batch 427 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-25T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch427 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([`err-${index}`], `status-batch427-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 218,
        samples: [{ checkedAt, status, failedChecks: ['batch427'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-25T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch427-${index}`,
  ] as const))(
    'generated batch427 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('z');
...
describe('release observation batch 428 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-26T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch428 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch428-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 219,
        samples: [{ checkedAt, status, failedChecks: ['batch428'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-26T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch428-${index}`,
  ] as const))(
    'generated batch428 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
...
describe('release observation batch 429 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-27T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch429 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch429-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 220,
        samples: [{ checkedAt, status, failedChecks: ['batch429'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-27T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch429-${index}`,
  ] as const))(
    'generated batch429 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.reverse();
...
describe('release observation batch 430 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-28T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch430 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch430-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 221,
        samples: [{ checkedAt, status, failedChecks: ['batch430'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-28T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch430-${index}`,
  ] as const))(
    'generated batch430 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
...
describe('release observation batch 431 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-29T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch431 URIError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new URIError(`status-batch431-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 222,
        samples: [{ checkedAt, status, failedChecks: ['batch431'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-29T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `copyWithin-batch431-${index}`,
  ] as const))(
    'generated batch431 failedChecks copyWithin after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a', 'd'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.copyWithin(0, 2, 3);
...
describe('release observation batch 432 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-30T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch432 ReferenceError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new ReferenceError(`status-batch432-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 223,
        samples: [{ checkedAt, status, failedChecks: ['batch432'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-30T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `fill-batch432-${index}`,
  ] as const))(
    'generated batch432 failedChecks fill after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.fill('x', 1, 2);
...
describe('release observation batch 433 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2026-12-31T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch433 EvalError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new EvalError(`status-batch433-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 224,
        samples: [{ checkedAt, status, failedChecks: ['batch433'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-31T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `splice-batch433-${index}`,
  ] as const))(
    'generated batch433 failedChecks splice after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.splice(1, 1, 'y');
...
describe('release observation batch 434 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-01T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch434 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([`err-${index}`], `status-batch434-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 225,
        samples: [{ checkedAt, status, failedChecks: ['batch434'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-01T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch434-${index}`,
  ] as const))(
    'generated batch434 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('z');
...
describe('release observation batch 435 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-02T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch435 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch435-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 226,
        samples: [{ checkedAt, status, failedChecks: ['batch435'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-02T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch435-${index}`,
  ] as const))(
    'generated batch435 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
...
describe('release observation batch 436 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-03T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch436 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch436-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 227,
        samples: [{ checkedAt, status, failedChecks: ['batch436'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-03T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch436-${index}`,
  ] as const))(
    'generated batch436 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.reverse();
...
describe('release observation batch 437 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-04T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch437 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch437-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 228,
        samples: [{ checkedAt, status, failedChecks: ['batch437'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-04T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch437-${index}`,
  ] as const))(
    'generated batch437 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['a', 'c', replacementFailure]);
    },
  );
});

describe('release observation batch 438 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-05T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch438 URIError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new URIError(`status-batch438-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 229,
        samples: [{ checkedAt, status, failedChecks: ['batch438'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-05T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `copyWithin-batch438-${index}`,
  ] as const))(
    'generated batch438 failedChecks copyWithin after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.copyWithin(0, 1, 2);
      ...
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual([replacementFailure, replacementFailure, 'a']);
    },
  );
});

describe('release observation batch 439 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-06T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch439 ReferenceError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new ReferenceError(`status-batch439-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 230,
        samples: [{ checkedAt, status, failedChecks: ['batch439'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-06T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `fill-batch439-${index}`,
  ] as const))(
    'generated batch439 failedChecks fill after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.fill('filled', 0, 1);

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['filled', replacementFailure, 'a']);
    },
  );
});

describe('release observation batch 440 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-07T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch440 EvalError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new EvalError(`status-batch440-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 231,
        samples: [{ checkedAt, status, failedChecks: ['batch440'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-07T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `splice-batch440-${index}`,
  ] as const))(
    'generated batch440 failedChecks splice after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.splice(1, 1);

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['c', 'a']);
    },
  );
});

describe('release observation batch 441 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-08T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch441 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([`err-${index}`], `status-batch441-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 232,
        samples: [{ checkedAt, status, failedChecks: ['batch441'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-08T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch441-${index}`,
  ] as const))(
    'generated batch441 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('added');

      expect(summary.status).toBe('ATTENTION_REQUIRED');
      expect(summary.failedSamples).toBe(1);
      expect(summary.firstFailureAt).toBe(checkedAt);
      expect(summary.samples[0].failedChecks).toEqual(['added', 'c', replacementFailure, 'a']);
    },
  );
});

describe('release observation batch 442 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-09T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch442 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch442-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 233,
        samples: [{ checkedAt, status, failedChecks: ['batch442'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-09T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch442-${index}`,
  ] as const))(
    'generated batch442 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
...
describe('release observation batch 443 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-10T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch443 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch443-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 234,
        samples: [{ checkedAt, status, failedChecks: ['batch443'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-10T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch443-${index}`,
  ] as const))(
    'generated batch443 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.reverse();
...
describe('release observation batch 444 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-11T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch444 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch444-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 235,
        samples: [{ checkedAt, status, failedChecks: ['batch444'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-11T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch444-${index}`,
  ] as const))(
    'generated batch444 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
...
describe('release observation batch 445 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-12T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch445 URIError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new URIError(`status-batch445-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 236,
        samples: [{ checkedAt, status, failedChecks: ['batch445'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-12T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `copyWithin-batch445-${index}`,
  ] as const))(
    'generated batch445 failedChecks copyWithin after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.copyWithin(0, 1, 2);
...
describe('release observation batch 446 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-13T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch446 ReferenceError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new ReferenceError(`status-batch446-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 237,
        samples: [{ checkedAt, status, failedChecks: ['batch446'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-13T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `fill-batch446-${index}`,
  ] as const))(
    'generated batch446 failedChecks fill after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.fill('filled', 0, 1);
...
describe('release observation batch 447 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-14T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch447 EvalError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new EvalError(`status-batch447-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 238,
        samples: [{ checkedAt, status, failedChecks: ['batch447'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-14T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `splice-batch447-${index}`,
  ] as const))(
    'generated batch447 failedChecks splice after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.splice(1, 1);
...
describe('release observation batch 448 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-15T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch448 AggregateError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new AggregateError([`err-${index}`], `status-batch448-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 239,
        samples: [{ checkedAt, status, failedChecks: ['batch448'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-15T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `unshift-batch448-${index}`,
  ] as const))(
    'generated batch448 failedChecks unshift after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.unshift('added');
...
describe('release observation batch 449 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-16T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch449 Error status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new Error(`status-batch449-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 240,
        samples: [{ checkedAt, status, failedChecks: ['batch449'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-16T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `pop-batch449-${index}`,
  ] as const))(
    'generated batch449 failedChecks pop after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.pop();
...
describe('release observation batch 450 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-17T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch450 RangeError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new RangeError(`status-batch450-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 241,
        samples: [{ checkedAt, status, failedChecks: ['batch450'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-17T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `reverse-batch450-${index}`,
  ] as const))(
    'generated batch450 failedChecks reverse after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.reverse();
...
describe('release observation batch 451 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-18T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch451 SyntaxError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new SyntaxError(`status-batch451-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 242,
        samples: [{ checkedAt, status, failedChecks: ['batch451'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-18T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `sort-batch451-${index}`,
  ] as const))(
    'generated batch451 failedChecks sort after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.sort();
...
describe('release observation batch 452 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `2027-01-19T00:${String(index % 50).padStart(2, '0')}:00.000Z`,
    index,
  ] as const))(
    'generated batch452 URIError status is retained without failure count %s',
    (checkedAt, index) => {
      const status = new URIError(`status-batch452-${index}`) as unknown as 'NO_GO';
      const summary = summarizeReleaseObservation({
        windowMinutes: 243,
        samples: [{ checkedAt, status, failedChecks: ['batch452'] }],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-19T01:${String(index % 50).padStart(2, '0')}:00.000Z`,
    `copyWithin-batch452-${index}`,
  ] as const))(
    'generated batch452 failedChecks copyWithin after summary changes sample view only %#',
    (checkedAt, replacementFailure) => {
      const sample = { checkedAt, status: 'NO_GO' as const, failedChecks: ['c', replacementFailure, 'a'] };
      const summary = summarizeReleaseObservation({ windowMinutes: 1, samples: [sample] });
      sample.failedChecks.copyWithin(0, 1, 2);
