...
describe('release readiness batch 444 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new Set(), {
      status: 'ok',
      checks: { database: { status: new SyntaxError(`batch444-${index}`) } },
    }),
    String(new SyntaxError(`batch444-${index}`)),
  ] as const))(
    'generated batch444 Set health snapshot fails SyntaxError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new Set(), {
      status: 'ok',
      alerts: [{ id: new SyntaxError(`alert-batch444-${index}`) as unknown as string }],
    }),
    String(new SyntaxError(`alert-batch444-${index}`)),
  ] as const))(
    'generated batch444 Set metrics SyntaxError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 445 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new WeakSet(), {
      status: 'ok',
      checks: { database: { status: new URIError(`batch445-${index}`) } },
    }),
    String(new URIError(`batch445-${index}`)),
  ] as const))(
    'generated batch445 WeakSet health snapshot fails URIError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new WeakSet(), {
      status: 'ok',
      alerts: [{ id: new URIError(`alert-batch445-${index}`) as unknown as string }],
    }),
    String(new URIError(`alert-batch445-${index}`)),
  ] as const))(
    'generated batch445 WeakSet metrics URIError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 446 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new Date(Date.UTC(2026, 4, 16, index % 24, 0, 0)), {
      status: 'ok',
      checks: { database: { status: new ReferenceError(`batch446-${index}`) } },
    }),
    String(new ReferenceError(`batch446-${index}`)),
  ] as const))(
    'generated batch446 Date health snapshot fails ReferenceError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new Date(Date.UTC(2026, 4, 16, index % 24, 0, 0)), {
      status: 'ok',
      alerts: [{ id: new ReferenceError(`alert-batch446-${index}`) as unknown as string }],
    }),
    String(new ReferenceError(`alert-batch446-${index}`)),
  ] as const))(
    'generated batch446 Date metrics ReferenceError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 447 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new RegExp(`batch447-${index}`), {
      status: 'ok',
      checks: { database: { status: new EvalError(`batch447-${index}`) } },
    }),
    String(new EvalError(`batch447-${index}`)),
  ] as const))(
    'generated batch447 RegExp health snapshot fails EvalError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new RegExp(`alert-batch447-${index}`), {
      status: 'ok',
      alerts: [{ id: new EvalError(`alert-batch447-${index}`) as unknown as string }],
    }),
    String(new EvalError(`alert-batch447-${index}`)),
  ] as const))(
    'generated batch447 RegExp metrics EvalError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 448 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(Promise.resolve('ok'), {
      status: 'ok',
      checks: { database: { status: new AggregateError([`err-${index}`], `batch448-${index}`) } },
    }),
    String(new AggregateError([`err-${index}`], `batch448-${index}`)),
  ] as const))(
    'generated batch448 Promise health snapshot fails AggregateError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(Promise.resolve('ok'), {
      status: 'ok',
      alerts: [{ id: new AggregateError([`err-${index}`], `alert-batch448-${index}`) as unknown as string }],
    }),
    String(new AggregateError([`err-${index}`], `alert-batch448-${index}`)),
  ] as const))(
    'generated batch448 Promise metrics AggregateError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 449 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new WeakMap(), {
      status: 'ok',
      checks: { database: { status: new Error(`batch449-${index}`) } },
    }),
    String(new Error(`batch449-${index}`)),
  ] as const))(
    'generated batch449 WeakMap health snapshot fails Error database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new WeakMap(), {
      status: 'ok',
      alerts: [{ id: new Error(`alert-batch449-${index}`) as unknown as string }],
    }),
    String(new Error(`alert-batch449-${index}`)),
  ] as const))(
    'generated batch449 WeakMap metrics Error alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 450 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new Map(), {
      status: 'ok',
      checks: { database: { status: new RangeError(`batch450-${index}`) } },
    }),
    String(new RangeError(`batch450-${index}`)),
  ] as const))(
    'generated batch450 Map health snapshot fails RangeError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new Map(), {
      status: 'ok',
      alerts: [{ id: new RangeError(`alert-batch450-${index}`) as unknown as string }],
    }),
    String(new RangeError(`alert-batch450-${index}`)),
  ] as const))(
    'generated batch450 Map metrics RangeError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
...
describe('release readiness batch 451 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    Object.assign(new Set(), {
      status: 'ok',
      checks: { database: { status: new SyntaxError(`batch451-${index}`) } },
    }),
    String(new SyntaxError(`batch451-${index}`)),
  ] as const))(
    'generated batch451 Set health snapshot fails SyntaxError database status %#',
    (health, databaseStatus) => {
      const report = evaluateReleaseReadiness({
        health,
        metrics: { status: 'ok', alerts: [] },
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    Object.assign(new Set(), {
      status: 'ok',
      alerts: [{ id: new SyntaxError(`alert-batch451-${index}`) as unknown as string }],
    }),
    String(new SyntaxError(`alert-batch451-${index}`)),
  ] as const))(
    'generated batch451 Set metrics SyntaxError alert id is reported %#',
    (metrics, alertId) => {
      const report = evaluateReleaseReadiness({
        health: { status: 'ok', checks: { database: { status: 'ok' } } },
        metrics,
