...
describe('canary report batch 422 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new RangeError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch422-${index}` },
  ] as const))(
    'generated batch422 RangeError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.231.0',
        targetVersion: '25.230.0',
        startedAt: '2026-12-20T03:00:00.000Z',
        endedAt: '2026-12-20T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-20T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch422 eleven thousand ten second duration rounds to one hundred eighty four minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11010000).toISOString();
      const report = buildCanaryReport({
        version: '25.231.1',
        targetVersion: '25.230.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(184);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 423 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new SyntaxError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch423-${index}` },
  ] as const))(
    'generated batch423 SyntaxError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.232.0',
        targetVersion: '25.231.0',
        startedAt: '2026-12-21T03:00:00.000Z',
        endedAt: '2026-12-21T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-21T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch423 eleven thousand seventy second duration rounds to one hundred eighty five minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11070000).toISOString();
      const report = buildCanaryReport({
        version: '25.232.1',
        targetVersion: '25.231.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(185);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 424 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new URIError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch424-${index}` },
  ] as const))(
    'generated batch424 URIError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.233.0',
        targetVersion: '25.232.0',
        startedAt: '2026-12-22T03:00:00.000Z',
        endedAt: '2026-12-22T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-22T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch424 eleven thousand one hundred thirty second duration rounds to one hundred eighty six minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11130000).toISOString();
      const report = buildCanaryReport({
        version: '25.233.1',
        targetVersion: '25.232.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(186);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 425 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new ReferenceError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch425-${index}` },
  ] as const))(
    'generated batch425 ReferenceError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.234.0',
        targetVersion: '25.233.0',
        startedAt: '2026-12-23T03:00:00.000Z',
        endedAt: '2026-12-23T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-23T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch425 eleven thousand one hundred ninety second duration rounds to one hundred eighty seven minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11190000).toISOString();
      const report = buildCanaryReport({
        version: '25.234.1',
        targetVersion: '25.233.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(187);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 426 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new EvalError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch426-${index}` },
  ] as const))(
    'generated batch426 EvalError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.235.0',
        targetVersion: '25.234.0',
        startedAt: '2026-12-24T03:00:00.000Z',
        endedAt: '2026-12-24T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-24T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch426 eleven thousand two hundred fifty second duration rounds to one hundred eighty eight minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11250000).toISOString();
      const report = buildCanaryReport({
        version: '25.235.1',
        targetVersion: '25.234.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(188);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 427 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new AggregateError([`err-${index}`], `FAIL-${index}`) as unknown as 'FAIL', note: `batch427-${index}` },
  ] as const))(
    'generated batch427 AggregateError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.236.0',
        targetVersion: '25.235.0',
        startedAt: '2026-12-25T03:00:00.000Z',
        endedAt: '2026-12-25T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-25T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch427 eleven thousand three hundred ten second duration rounds to one hundred eighty nine minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11310000).toISOString();
      const report = buildCanaryReport({
        version: '25.236.1',
        targetVersion: '25.235.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(189);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 428 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new Error(`FAIL-${index}`) as unknown as 'FAIL', note: `batch428-${index}` },
  ] as const))(
    'generated batch428 Error failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.237.0',
        targetVersion: '25.236.0',
        startedAt: '2026-12-26T03:00:00.000Z',
        endedAt: '2026-12-26T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-26T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch428 eleven thousand three hundred seventy second duration rounds to one hundred ninety minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11370000).toISOString();
      const report = buildCanaryReport({
        version: '25.237.1',
        targetVersion: '25.236.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(190);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 429 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new RangeError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch429-${index}` },
  ] as const))(
    'generated batch429 RangeError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.238.0',
        targetVersion: '25.237.0',
        startedAt: '2026-12-27T03:00:00.000Z',
        endedAt: '2026-12-27T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-27T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch429 eleven thousand four hundred thirty second duration rounds to one hundred ninety one minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11430000).toISOString();
      const report = buildCanaryReport({
        version: '25.238.1',
        targetVersion: '25.237.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(191);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 430 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new SyntaxError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch430-${index}` },
  ] as const))(
    'generated batch430 SyntaxError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.239.0',
        targetVersion: '25.238.0',
        startedAt: '2026-12-28T03:00:00.000Z',
        endedAt: '2026-12-28T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-28T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch430 eleven thousand four hundred ninety second duration rounds to one hundred ninety two minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11490000).toISOString();
      const report = buildCanaryReport({
        version: '25.239.1',
        targetVersion: '25.238.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(192);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 431 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new URIError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch431-${index}` },
  ] as const))(
    'generated batch431 URIError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.240.0',
        targetVersion: '25.239.0',
        startedAt: '2026-12-29T03:00:00.000Z',
        endedAt: '2026-12-29T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-29T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch431 eleven thousand five hundred fifty second duration rounds to one hundred ninety three minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11550000).toISOString();
      const report = buildCanaryReport({
        version: '25.240.1',
        targetVersion: '25.239.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(193);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 432 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new ReferenceError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch432-${index}` },
  ] as const))(
    'generated batch432 ReferenceError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.241.0',
        targetVersion: '25.240.0',
        startedAt: '2026-12-30T03:00:00.000Z',
        endedAt: '2026-12-30T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-30T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch432 eleven thousand six hundred ten second duration rounds to one hundred ninety four minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11610000).toISOString();
      const report = buildCanaryReport({
        version: '25.241.1',
        targetVersion: '25.240.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(194);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 433 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new EvalError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch433-${index}` },
  ] as const))(
    'generated batch433 EvalError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.242.0',
        targetVersion: '25.241.0',
        startedAt: '2026-12-31T03:00:00.000Z',
        endedAt: '2026-12-31T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2026-12-31T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch433 eleven thousand six hundred seventy second duration rounds to one hundred ninety five minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11670000).toISOString();
      const report = buildCanaryReport({
        version: '25.242.1',
        targetVersion: '25.241.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(195);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 434 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new AggregateError([`err-${index}`], `FAIL-${index}`) as unknown as 'FAIL', note: `batch434-${index}` },
  ] as const))(
    'generated batch434 AggregateError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.243.0',
        targetVersion: '25.242.0',
        startedAt: '2027-01-01T03:00:00.000Z',
        endedAt: '2027-01-01T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-01T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch434 eleven thousand seven hundred thirty second duration rounds to one hundred ninety six minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11730000).toISOString();
      const report = buildCanaryReport({
        version: '25.243.1',
        targetVersion: '25.242.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(196);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 435 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_5', status: new Error(`FAIL-${index}`) as unknown as 'FAIL', note: `batch435-${index}` },
  ] as const))(
    'generated batch435 Error failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.244.0',
        targetVersion: '25.243.0',
        startedAt: '2027-01-02T03:00:00.000Z',
        endedAt: '2027-01-02T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-02T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch435 eleven thousand seven hundred ninety second duration rounds to one hundred ninety seven minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11790000).toISOString();
      const report = buildCanaryReport({
        version: '25.244.1',
        targetVersion: '25.243.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(197);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 436 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_6', status: new RangeError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch436-${index}` },
  ] as const))(
    'generated batch436 RangeError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.245.0',
        targetVersion: '25.244.0',
        startedAt: '2027-01-03T03:00:00.000Z',
        endedAt: '2027-01-03T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-03T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch436 eleven thousand eight hundred fifty second duration rounds to one hundred ninety eight minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11850000).toISOString();
      const report = buildCanaryReport({
        version: '25.245.1',
        targetVersion: '25.244.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(198);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 437 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_7', status: new SyntaxError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch437-${index}` },
  ] as const))(
    'generated batch437 SyntaxError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.246.0',
        targetVersion: '25.245.0',
        startedAt: '2027-01-04T03:00:00.000Z',
        endedAt: '2027-01-04T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-04T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch437 eleven thousand nine hundred ten second duration rounds to one hundred ninety nine minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11910000).toISOString();
      const report = buildCanaryReport({
        version: '25.246.1',
        targetVersion: '25.245.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(199);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 438 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_8', status: new URIError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch438-${index}` },
  ] as const))(
    'generated batch438 URIError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.247.0',
        targetVersion: '25.246.0',
        startedAt: '2027-01-05T03:00:00.000Z',
        endedAt: '2027-01-05T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-05T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch438 eleven thousand nine hundred seventy second duration rounds to two hundred minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 11970000).toISOString();
      const report = buildCanaryReport({
        version: '25.247.1',
        targetVersion: '25.246.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(200);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 439 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_9', status: new ReferenceError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch439-${index}` },
  ] as const))(
    'generated batch439 ReferenceError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.248.0',
        targetVersion: '25.247.0',
        startedAt: '2027-01-06T03:00:00.000Z',
        endedAt: '2027-01-06T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-06T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch439 twelve thousand thirty second duration rounds to two hundred one minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12030000).toISOString();
      const report = buildCanaryReport({
        version: '25.248.1',
        targetVersion: '25.247.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(201);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 440 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_10', status: new EvalError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch440-${index}` },
  ] as const))(
    'generated batch440 EvalError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.249.0',
        targetVersion: '25.248.0',
        startedAt: '2027-01-07T03:00:00.000Z',
        endedAt: '2027-01-07T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-07T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch440 twelve thousand ninety second duration rounds to two hundred two minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12090000).toISOString();
      const report = buildCanaryReport({
        version: '25.249.1',
        targetVersion: '25.248.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(202);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 441 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_11', status: new AggregateError([`err-${index}`], `FAIL-${index}`) as unknown as 'FAIL', note: `batch441-${index}` },
  ] as const))(
    'generated batch441 AggregateError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.250.0',
        targetVersion: '25.249.0',
        startedAt: '2027-01-08T03:00:00.000Z',
        endedAt: '2027-01-08T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-08T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch441 twelve thousand one hundred fifty second duration rounds to two hundred three minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12150000).toISOString();
      const report = buildCanaryReport({
        version: '25.250.1',
        targetVersion: '25.249.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(203);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 442 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_12', status: new Error(`FAIL-${index}`) as unknown as 'FAIL', note: `batch442-${index}` },
  ] as const))(
    'generated batch442 Error failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.251.0',
        targetVersion: '25.250.0',
        startedAt: '2027-01-09T03:00:00.000Z',
        endedAt: '2027-01-09T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-09T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch442 twelve thousand two hundred ten second duration rounds to two hundred four minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12210000).toISOString();
      const report = buildCanaryReport({
        version: '25.251.1',
        targetVersion: '25.250.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(204);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 443 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_13', status: new RangeError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch443-${index}` },
  ] as const))(
    'generated batch443 RangeError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.252.0',
        targetVersion: '25.251.0',
        startedAt: '2027-01-10T03:00:00.000Z',
        endedAt: '2027-01-10T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-10T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch443 twelve thousand two hundred seventy second duration rounds to two hundred five minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12270000).toISOString();
      const report = buildCanaryReport({
        version: '25.252.1',
        targetVersion: '25.251.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(205);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 444 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_14', status: new SyntaxError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch444-${index}` },
  ] as const))(
    'generated batch444 SyntaxError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.253.0',
        targetVersion: '25.252.0',
        startedAt: '2027-01-11T03:00:00.000Z',
        endedAt: '2027-01-11T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-11T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch444 twelve thousand three hundred thirty second duration rounds to two hundred six minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12330000).toISOString();
      const report = buildCanaryReport({
        version: '25.253.1',
        targetVersion: '25.252.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(206);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 445 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_15', status: new URIError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch445-${index}` },
  ] as const))(
    'generated batch445 URIError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.254.0',
        targetVersion: '25.253.0',
        startedAt: '2027-01-12T03:00:00.000Z',
        endedAt: '2027-01-12T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-12T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch445 twelve thousand three hundred ninety second duration rounds to two hundred seven minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12390000).toISOString();
      const report = buildCanaryReport({
        version: '25.254.1',
        targetVersion: '25.253.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(207);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 446 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_16', status: new ReferenceError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch446-${index}` },
  ] as const))(
    'generated batch446 ReferenceError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.255.0',
        targetVersion: '25.254.0',
        startedAt: '2027-01-13T03:00:00.000Z',
        endedAt: '2027-01-13T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-13T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch446 twelve thousand four hundred fifty second duration rounds to two hundred eight minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12450000).toISOString();
      const report = buildCanaryReport({
        version: '25.255.1',
        targetVersion: '25.254.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(208);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 447 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_17', status: new EvalError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch447-${index}` },
  ] as const))(
    'generated batch447 EvalError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.256.0',
        targetVersion: '25.255.0',
        startedAt: '2027-01-14T03:00:00.000Z',
        endedAt: '2027-01-14T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-14T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch447 twelve thousand five hundred ten second duration rounds to two hundred nine minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12510000).toISOString();
      const report = buildCanaryReport({
        version: '25.256.1',
        targetVersion: '25.255.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(209);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 448 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_18', status: new AggregateError([`err-${index}`], `FAIL-${index}`) as unknown as 'FAIL', note: `batch448-${index}` },
  ] as const))(
    'generated batch448 AggregateError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.257.0',
        targetVersion: '25.256.0',
        startedAt: '2027-01-15T03:00:00.000Z',
        endedAt: '2027-01-15T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-15T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch448 twelve thousand five hundred seventy second duration rounds to two hundred ten minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12570000).toISOString();
      const report = buildCanaryReport({
        version: '25.257.1',
        targetVersion: '25.256.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(210);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 449 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_19', status: new Error(`FAIL-${index}`) as unknown as 'FAIL', note: `batch449-${index}` },
  ] as const))(
    'generated batch449 Error failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.258.0',
        targetVersion: '25.257.0',
        startedAt: '2027-01-16T03:00:00.000Z',
        endedAt: '2027-01-16T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-16T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch449 twelve thousand six hundred thirty second duration rounds to two hundred eleven minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12630000).toISOString();
      const report = buildCanaryReport({
        version: '25.258.1',
        targetVersion: '25.257.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(211);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 450 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_20', status: new RangeError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch450-${index}` },
  ] as const))(
    'generated batch450 RangeError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.259.0',
        targetVersion: '25.258.0',
        startedAt: '2027-01-17T03:00:00.000Z',
        endedAt: '2027-01-17T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-17T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch450 twelve thousand six hundred ninety second duration rounds to two hundred twelve minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12690000).toISOString();
      const report = buildCanaryReport({
        version: '25.259.1',
        targetVersion: '25.258.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(212);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 451 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_21', status: new SyntaxError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch451-${index}` },
  ] as const))(
    'generated batch451 SyntaxError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.260.0',
        targetVersion: '25.259.0',
        startedAt: '2027-01-18T03:00:00.000Z',
        endedAt: '2027-01-18T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-18T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch451 twelve thousand seven hundred fifty second duration rounds to two hundred thirteen minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12750000).toISOString();
      const report = buildCanaryReport({
        version: '25.260.1',
        targetVersion: '25.259.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(213);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});

describe('canary report batch 452 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    { id: 'canary_22', status: new URIError(`FAIL-${index}`) as unknown as 'FAIL', note: `batch452-${index}` },
  ] as const))(
    'generated batch452 URIError failed stage status is not treated as failure %#',
    (stage) => {
      const report = buildCanaryReport({
        version: '25.261.0',
        targetVersion: '25.260.0',
        startedAt: '2027-01-19T03:00:00.000Z',
        endedAt: '2027-01-19T03:01:00.000Z',
        stages: [stage as CanaryStageResult],
      });
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `2027-01-19T04:${String(index % 50).padStart(2, '0')}:00.000Z`,
  ] as const))(
    'generated batch452 twelve thousand eight hundred ten second duration rounds to two hundred fourteen minutes %s',
    (startedAt) => {
      const endedAt = new Date(new Date(startedAt).getTime() + 12810000).toISOString();
      const report = buildCanaryReport({
        version: '25.261.1',
        targetVersion: '25.260.1',
        startedAt,
        endedAt,
        stages: [{ id: 'full_rollout', status: 'PASS' }],
      });
      ...
      expect(report.status).toBe('COMPLETED');
      expect(report.durationMinutes).toBe(214);
      expect(report.firstFailedStage).toBeNull();
    },
  );
});
