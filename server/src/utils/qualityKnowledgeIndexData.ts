import { KnowledgeIndexSection } from './qualityKnowledgeIndex';

/**
 * 质量知识库的预置索引数据（与 atlas-quality-system/docs 一一对应）。
 *
 * 抽到独立模块，便于脚本（writeQualityKnowledgeIndex.ts）与测试共用——
 * 测试据此断言「磁盘上每个 docs/*.md 都已登记」（见 findUnindexedDocs），
 * 防止 38/39/40 那种「新文档没进索引」的静默漂移。
 *
 * ⚠️ 新增 atlas-quality-system/docs/*.md 时必须在此登记，否则
 * `npm run quality:knowledge-index` 与 qualityKnowledgeIndexData 单测都会失败。
 */
export const sections: KnowledgeIndexSection[] = [
  {
    name: '基础方案',
    items: [
