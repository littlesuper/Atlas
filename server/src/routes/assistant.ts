 ...
 * 全系统 AI 助手 · 通用路由
 *
 * POST /api/assistant/propose { utterance, contextProjectId } → 预览（不写库）
 * POST /api/assistant/apply   { proposalId }                  → 经现有校验路由落库 + 审计
 *
 * 所有写操作统一走能力层（capability）；只读提问走 runAsk。
 * 旧 adapter 体系已在 Task 5 删除。
 */
...
import { capabilityPropose, capabilityApply, CapabilityForbiddenError } from '../services/assistant/capability/orchestrator';
import {
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      // 能力分支：命中某个 Capability（project.create / schedule.update / project.update / risk.update 等）
