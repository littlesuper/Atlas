...
describe('Weekly Reports Routes', () => {
  ...
  describe('POST /api/weekly-reports', () => {
    ...
    it('GLM QA bug repro #5: 同周重复创建应返回 409（P2002 唯一约束），而非被 catch 吞成 500（WR-001）', async () => {
      ...
      // 现状：创建处理器的 catch 不区分 P2002，统一返回 500「服务器内部错误」——
      // 既违反 WR-001 的状态码契约，又把可预期的业务冲突伪装成服务端故障。
      mockPrisma.project.findUnique.mockResolvedValue(sampleProject);
      const p2002 = Object.assign(
        new Error('Unique constraint failed on the fields: (`projectId`,`year`,`weekNumber`)'),
        { code: 'P2002' }
      );
      ...
      expect(res.body).toHaveProperty('error', '本周该项目已存在周报');
    });
  });
