...
async function main() {
  ...
  const activity1 = await prisma.activity.create({
    data: {
      ...
      duration: 23,
      executors: { create: [{ userId: zhangsanUser.id, source: 'MANUAL_ADD' }] },
      sortOrder: 1,
    },
  });
  ...
  for (let i = 0; i < p2Activities.length; i++) {
    ...
    await prisma.activity.create({
      data: {
        ...
        duration: a.dur,
        executors: { create: [{ userId: a.assignee === 'z' ? z : l, source: 'MANUAL_ADD' }] },
        sortOrder: (i + 1) * 10,
        notes: a.notes,
      },
    });
  ...
  const p3a1 = await prisma.activity.create({
    data: {
      ...
      planDuration: 33,
      startDate: new Date('2026-03-01'),
      executors: { create: [{ userId: lisiUser.id, source: 'MANUAL_ADD' }] },
      sortOrder: 10,
    },
  });
