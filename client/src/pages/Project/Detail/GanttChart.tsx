...
const GanttChart: React.FC<GanttProps> = ({ activities, criticalActivityIds = [] }) => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (!entry.isIntersecting) setHeaderFixed(false);
      },
      { threshold: 0 }
    );
  ...
  // 通用分组行渲染
  const renderGroupRow = (groups: Array<{ label: string; days: number }>, h: number, fs: number, bg?: string) => (
    <div style={{ display: 'flex', height: h, borderBottom: '1px solid var(--border)', background: bg }}>
      {groups.map((g, i) => (
        <div
          key={i}
          style={{
  ...
  const renderHeader = () => {
    switch (viewMode) {
      ...
      case 'week': {
        ...
        return (
          <>
            {renderGroupRow(monthGroups, HEADER_TOP, 10, 'var(--accent)')}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      textAlign: 'center',
                      fontSize: 11,
                      color: isToday ? TODAY_COLOR : isWeekend ? 'var(--muted-foreground)' : 'var(--foreground)',
                      borderRight: '1px solid var(--border)',
                      fontWeight: isToday ? 700 : undefined,
                      background: isWeekend ? 'var(--muted)' : undefined,
                      display: 'flex',
                      alignItems: 'center',
  ...
  if (activities.length === 0) {
    return <div className="text-muted-foreground p-10 text-center text-sm">暂无活动数据，请先创建活动</div>;
  }
    ...
    <>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 py-2">
        <div className="flex items-center gap-1">
          {(['day', 'week', 'month', 'quarter', 'year'] as ViewMode[]).map((m) => (
            <Button key={m} size="sm" variant={viewMode === m ? 'default' : 'outline'} onClick={() => setViewMode(m)}>
              {VIEW_CONFIG[m].label}
            </Button>
          ))}
        </div>
        <div className="flex items-center gap-2">
          <DateRangePicker
            ...
            value={customRange ? { from: customRange[0].toDate(), to: customRange[1].toDate() } : undefined}
            onChange={(range?: DateRange) => {
              if (range?.from && range?.to) {
                setCustomRange([dayjs(range.from), dayjs(range.to)]);
              } else {
                setCustomRange(null);
              }
            }}
          ...
          />
          {customRange && (
            <Button size="icon" variant="outline" className="size-8" aria-label="重置时间范围" onClick={() => setCustomRange(null)}>
      ...
      <div className="flex flex-wrap items-center gap-3 pb-2">
        {LEGEND_ITEMS.map((item, i) => (
          <div key={i} className="flex items-center gap-1 text-xs" style={{ color: item.color || 'var(--muted-foreground)' }}>
            {item.icon}
      ...
      <div
        style={{
          ...
          borderBottom: 'none',
          borderRadius: isFixed ? 0 : '6px 6px 0 0',
      ...
      >
        ...
        <div ref={scrollRef as React.Ref<HTMLDivElement>} style={{ overflow: 'hidden', flex: 1 }}>
          <div style={{ width: totalWidth, minWidth: '100%', background: 'var(--muted)' }}>{renderHeader()}</div>
        </div>
      </div>
    </>
  );
  ...
  return (
    <TooltipProvider>
      <div ref={outerRef}>
        {/* ===== 普通流表头：始终保留空间，未固定时可见 ===== */}
        <div style={{ visibility: headerFixed ? 'hidden' : 'visible', background: 'var(--background)' }}>
          {renderHeaderBlock(headerScrollRef)}
        </div>

        {/* ===== 吸顶复制表头：portal 到 body ===== */}
        {headerFixed &&
          createPortal(
            <div
              style={{
        ...
        {/* ===== 活动行区域 ===== */}
        <div style={{ display: 'flex', border: '1px solid var(--border)', borderTop: 'none', borderRadius: '0 0 6px 6px' }}>
          {/* 左侧标签列 */}
          <div
            style={{
              width: LABEL_COL_W,
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    paddingRight: 8,
                    borderBottom: '1px solid var(--border)',
                    background: hoveredId === a.id ? 'var(--accent)' : idx % 2 === 0 ? 'var(--background)' : 'var(--muted)',
                    fontSize: 12,
                    gap: 6,
                    overflow: 'hidden',
                  }}
                ...
                >
                  ...
                  <Badge variant="outline" className={cn('px-1 py-0 text-[10px] leading-4', arcoBadgeClass(statusCfg.color))}>
                    {statusCfg.label}
                  </Badge>
                </div>
              );
          ...
          {/* 右侧横向滚动区 */}
          <div ref={bodyRef} onScroll={syncHeader} style={{ flex: 1, overflowX: 'auto', overflowY: 'visible' }}>
            <div style={{ width: totalWidth, minWidth: '100%', position: 'relative' }}>
              {/* 依赖箭头 */}
              <GanttArrows activities={activities} rangeStart={rangeStart} dayWidth={dayWidth} />
              {/* 今天竖线 */}
              {todayX >= 0 && todayX <= totalWidth && (
                <div style={{ position: 'absolute', left: todayX, top: 0, bottom: 0, width: 2, background: TODAY_COLOR, zIndex: 5, opacity: 0.8 }} />
              )}
                  ...
                  <Tooltip key={a.id}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        {actualBar && !isMilestone && (
                          ...
                          >
                            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${progress}%`, background: color, borderRadius: 3 }} />
                          </div>
                        )}
                    ...
                    <TooltipContent side="right">
                      <div className="text-xs leading-relaxed">
                        ...
                        {a.planStartDate && (
                          <div>
                            计划：{dayjs(a.planStartDate).format('MM-DD')} ~ {a.planEndDate ? dayjs(a.planEndDate).format('MM-DD') : '?'} (
                            {a.planDuration || '-'}工作日)
                          </div>
                        )}
                        {a.startDate && (
                          <div>
                            实际：{dayjs(a.startDate).format('MM-DD')} ~ {a.endDate ? dayjs(a.endDate).format('MM-DD') : '进行中'} (
                            {a.duration || '-'}工作日)
                          </div>
                        )}
                        <div>状态：{ACTIVITY_STATUS_MAP[a.status as keyof typeof ACTIVITY_STATUS_MAP]?.label || a.status}</div>
                        {a.executors?.length ? (
                          <div>负责人：{a.executors.map((e) => e.user?.realName).filter(Boolean).join(', ')}</div>
                        ) : null}
                      </div>
                    </TooltipContent>
                  </Tooltip>
