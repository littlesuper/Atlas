...
describe('monitoring batch 443 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch443-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch443 reverse RangeError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '151.125');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch443-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch443 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '151.125');
      vi.resetModules();
...
describe('monitoring batch 444 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch444-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch444 sort SyntaxError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '151.875');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch444-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch444 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '151.875');
      vi.resetModules();
...
describe('monitoring batch 445 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch445-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch445 copyWithin URIError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '152.625');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch445-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch445 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '152.625');
      vi.resetModules();
...
describe('monitoring batch 446 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch446-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch446 fill ReferenceError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '153.375');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch446-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch446 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '153.375');
      vi.resetModules();
...
describe('monitoring batch 447 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch447-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch447 splice EvalError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '154.125');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch447-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch447 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '154.125');
      vi.resetModules();
...
describe('monitoring batch 448 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch448-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch448 unshift AggregateError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '154.875');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch448-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch448 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '154.875');
      vi.resetModules();
...
describe('monitoring batch 449 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch449-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch449 pop Error dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '155.625');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch449-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch449 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '155.625');
      vi.resetModules();
...
describe('monitoring batch 450 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch450-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch450 reverse RangeError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '156.375');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch450-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch450 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '156.375');
      vi.resetModules();
...
describe('monitoring batch 451 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch451-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch451 sort SyntaxError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '157.125');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch451-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch451 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '157.125');
      vi.resetModules();
...
describe('monitoring batch 452 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `https://batch452-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch452 copyWithin URIError dsn as valid %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '157.875');
      vi.resetModules();
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `https://batch452-alt-${index}@sentry.io/${index}`,
  ] as const))(
    'initMonitoring treats generated batch452 fractional sample rate as finite %s',
    async (dsn) => {
      vi.stubEnv('VITE_SENTRY_DSN', dsn);
      vi.stubEnv('VITE_SENTRY_TRACES_SAMPLE_RATE', '157.875');
      vi.resetModules();
