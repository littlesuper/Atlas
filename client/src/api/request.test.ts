...
describe('request batch 441 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `AggregateError: batch441 ${index} occurred`,
    index,
  ] as const))(
    'getErrorMessage extracts batch441 AggregateError response data message %s',
    async (message, index) => {
      const errorText = new AggregateError([`err-${index}`], `err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch441 ${index}`,
    index,
  ] as const))(
    'getErrorMessage gives generated batch441 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 442 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `Error: batch442 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch442 Error response data message %s',
    async (message) => {
      const errorText = new Error(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch442 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch442 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 443 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `RangeError: batch443 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch443 RangeError response data message %s',
    async (message) => {
      const errorText = new RangeError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch443 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch443 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 444 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `SyntaxError: batch444 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch444 SyntaxError response data message %s',
    async (message) => {
      const errorText = new SyntaxError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch444 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch444 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 445 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `URIError: batch445 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch445 URIError response data message %s',
    async (message) => {
      const errorText = new URIError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch445 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch445 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 446 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `ReferenceError: batch446 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch446 ReferenceError response data message %s',
    async (message) => {
      const errorText = new ReferenceError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch446 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch446 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 447 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `EvalError: batch447 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch447 EvalError response data message %s',
    async (message) => {
      const errorText = new EvalError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch447 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch447 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 448 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `AggregateError: batch448 ${index} occurred`,
    index,
  ] as const))(
    'getErrorMessage extracts batch448 AggregateError response data message %s',
    async (message, index) => {
      const errorText = new AggregateError([`err-${index}`], `err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch448 ${index}`,
    index,
  ] as const))(
    'getErrorMessage gives generated batch448 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 449 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `Error: batch449 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch449 Error response data message %s',
    async (message) => {
      const errorText = new Error(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch449 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch449 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 450 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `RangeError: batch450 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch450 RangeError response data message %s',
    async (message) => {
      const errorText = new RangeError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch450 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch450 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 451 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `SyntaxError: batch451 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch451 SyntaxError response data message %s',
    async (message) => {
      const errorText = new SyntaxError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch451 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch451 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
...
describe('request batch 452 matrices', () => {
  it.each(Array.from({ length: 80 }, (_, index) => [
    `URIError: batch452 ${index} occurred`,
  ] as const))(
    'getErrorMessage extracts batch452 URIError response data message %s',
    async (message) => {
      const errorText = new URIError(`err-${message}`).toString();
      const { getErrorMessage } = await import('./request');
      const error = {
  ...
  it.each(Array.from({ length: 60 }, (_, index) => [
    `timeout Network Error batch452 ${index}`,
  ] as const))(
    'getErrorMessage gives generated batch452 timeout before network error %s',
    async (message) => {
      const { getErrorMessage } = await import('./request');
