...
**前端实现:**
- 账号管理页面（Tab 切换）
- 用户管理表格（搜索、编辑、删除；角色列单击进入内联编辑，多选 + 搜索过滤，点击外部保存）
- 用户创建/编辑 Modal
- 角色管理表格
...
- `GET /api/activities/project/:projectId/export-excel` - 导出活动 Excel（exceljs）
- `POST /api/activities/project/:projectId/import-excel` - 从 Excel 批量导入活动（xlsx 解析；支持类型/前置依赖/实际开始结束的往返还原）
- `POST /api/activities/project/:projectId/undo-import` - 撤销最近一次导入
...
| `/login` | 登录页 | 无 |
| `/` | 重定向到 `/projects` | 需登录 |
| `/projects` | 项目列表（首页，含新建/编辑抽屉） | 需登录 |
| `/projects?new=1` | 项目列表自动打开「新建项目」抽屉 | 需 `project:create` 权限 |
| `/projects?edit=:id` | 项目列表自动打开「编辑项目」抽屉 | 需 `project:update` 权限 |
| `/projects/new` | 兼容旧地址，重定向到 `/projects?new=1` | 需 `project:create` 权限 |
| `/projects/:id/edit` | 兼容旧地址，重定向到 `/projects?edit=:id` | 需 `project:update` 权限 |
