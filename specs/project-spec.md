...
- **Tab 切换：**
  - **活动列表：** 扁平表格（无树形展开），支持拖拽排序（通过拖拽手柄上下拖动行，松手后自动保存排序至数据库）。列依次为：拖拽手柄、ID（3位序号如001/002/003，按活动顺序自动生成）、前置（前置活动的3位序号）、阶段（EVT/DVT/PVT/MP Tag）、活动名称、类型、状态（宽度100px）、优先级、负责人（宽度110px）、计划时间、实际时间（超期显示红色）、备注、操作。新建/编辑以右侧抽屉（Drawer）方式展现，表单布局：第一行为阶段（必选）和活动名称；描述；类型/状态/优先级一行三列；「时间」分隔区下计划时间和实际时间上下排列（各含时间范围选择器和工期输入框，支持双向自动计算）；负责人（仅显示姓名）；前置依赖；备注。抽屉底部按钮行：左侧为"批量导入活动"按钮（IconUpload 图标，仅新建模式显示），右侧为"取消"和"创建/保存"按钮。
    ...
    **双击快速编辑功能：**
    - **支持字段：** 活动名称、状态、负责人、计划时间、计划工期、实际时间、前置依赖、备注
    - **交互方式：** 双击单元格进入编辑模式，失焦或按Enter保存，按Esc退出编辑
    - **编辑器类型：**
      - 活动名称：Input 输入框
      - 状态：Select 下拉选择器（选择后自动保存）
      - 负责人：Select 下拉选择器（支持搜索，选择后自动保存）
      - 计划时间/实际时间：DatePicker.RangePicker 日期范围选择器
      - 计划工期：InputNumber 数值输入框
    ...
    **归档管理：**
    ...
    - **归档管理抽屉（Drawer，宽度 960px）：**
      ...
      - 点击某条归档 → 展开为完整只读表格，列与活动列表一致：ID（序号）、阶段、活动名称、类型、状态、负责人、计划工期、计划时间、实际时间（超期红色）、备注。表格支持横向滚动，最大高度 360px
      - 再次点击同一条归档 → 收起表格
