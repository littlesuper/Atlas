...
- **Tab 切换：**
  - **活动列表：** 扁平表格（无树形展开），支持拖拽排序（通过拖拽手柄上下拖动行，松手后自动保存排序至数据库）。列依次为：拖拽手柄、ID（3位序号如001/002/003，按活动顺序自动生成）、前置（前置活动的3位序号）、阶段（EVT/DVT/PVT/MP Tag，宽度80px）、活动名称（自适应宽度）、类型（宽度80px）、状态（宽度100px）、负责人（宽度120px）、工期（宽度70px）、计划时间（合并显示开始-结束，格式 `YY年MM月DD日 - YY年MM月DD日`，宽度210px）、实际开始（格式 `YY年MM月DD日`，宽度115px）、实际结束（格式 `YY年MM月DD日`，宽度115px，超期显示红色）、备注、操作。表格单元格内边距为 `6px`（全局覆盖）。工具栏右侧提供一个蓝色的\"活动\"（IconNav图标）下拉组按钮，点击/悬停展示三个子菜单：\"新建活动\"（IconPlus 图标）、\"批量导入\"（IconUpload 图标）和\"导出活动\"（IconDownload 图标）。新建/编辑以右侧抽屉（Drawer）方式展现，表单布局：第一行为阶段（必选）和活动名称；描述；类型/状态/优先级一行三列；「时间」分隔区下计划时间和实际时间上下排列（各含时间范围选择器和工期输入框，支持双向自动计算）；负责人（仅显示姓名）；前置依赖；备注。抽屉底部按钮行：右侧为\"取消\"和\"创建/保存\"按钮。
    - **批量导入活动：** 点击后弹出文件选择器（`.xlsx/.xls`），上传后系统自动识别列并批量创建活动。成功后提示"导入成功，共 N 条活动（跳过 M 条重复），[撤销]"，若有自动创建的联系人账号则另行提示。导入完成后自动关闭抽屉，并刷新活动列表和项目数据。
    - **删除活动：** 删除父活动时自动级联删除所有子孙活动，确认弹窗提示"确定要删除活动及其所有子活动吗？"。删除成功后提示"已删除 [撤销]"，点击撤销可恢复删除前的活动数据。
    - **批量删除：** 勾选多行后点击批量删除按钮，确认后删除所有选中活动。删除成功后提示"已删除 N 个活动 [撤销]"，点击撤销通过批量创建接口快速恢复，同时立即清除提示防止重复点击。
    - **工具栏筛选区域（表格右上方）：**
