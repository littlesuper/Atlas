...
> **目的**：将活动的"执行人"绑定方式从「直接选具体用户」改造为「先绑定 RBAC 角色，再按用户管理中已分配的角色自动填入对应人员」。角色与执行人**并存**且都持久化，执行人在创建时按映射自动填全员、用户可手删，缺映射时软提示但允许留空，老数据完全迁移。
>
> **版本**: v1.1
> **状态**: 已实施
> **变更说明（v1.1）**: 移除独立的 RoleMember 映射机制，改为直接读取 UserRole 表。角色→用户的关系完全由"用户管理"中的角色分配决定，无需在"角色成员"Tab 中额外配置。
> **预估工时**: 15 工作日 ≈ 3 周
