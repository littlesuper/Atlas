...
> ℹ️ **测试用例当前权威源：[docs/qa/test-plan.md](../docs/qa/test-plan.md)**（380+ 用例 / 16 模块，随回归更新）。本文件为 API/功能五模块的历史快照，可能与现状不完全一致；归档规范见 [docs/README.md](../docs/README.md)。
...
| ACT-013 | 循环依赖检测-创建 | 项目有 A→B→C 依赖链 | POST 创建活动 C 依赖指向 A | 400，`"存在循环依赖，无法保存"` |
| ACT-014 | 循环依赖检测-更新 | A 依赖 B，B 依赖 C | PUT C 的 dependencies 指向 A | 400，`"存在循环依赖，无法保存"` |
| ACT-015 | 无循环依赖正常保存 | A 依赖 B | PUT C 的 dependencies 指向 B | 200，依赖正常建立 |
| ACT-016 | 自依赖检测 | 活动 A 存在 | PUT A 的 dependencies 指向 A | 400，`"存在循环依赖，无法保存"` |

### 3.3b 日期校验与高亮

| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| DATE-001 | 抽屉-计划结束早于开始 | 编辑活动抽屉打开 | 计划开始设为 3月10日，计划结束设为 3月1日 | `Message.error('结束时间不能早于开始时间')`，不保存 |
| DATE-002 | 抽屉-实际结束早于开始 | 编辑活动抽屉打开 | 实际开始设为 3月10日，实际结束设为 3月1日 | `Message.error('结束时间不能早于开始时间')`，不保存 |
| DATE-003 | 内联-计划开始晚于结束 | 活动有计划结束 3月10日 | 内联编辑计划开始为 3月15日 | `Message.error` 提示，编辑取消 |
| DATE-004 | 内联-计划结束早于开始 | 活动有计划开始 3月10日 | 内联编辑计划结束为 3月5日 | `Message.error` 提示，编辑取消 |
| DATE-005 | 内联-实际开始晚于结束 | 活动有实际结束 3月10日 | 内联编辑实际开始为 3月15日 | `Message.error` 提示，编辑取消 |
| DATE-006 | 内联-实际结束早于开始 | 活动有实际开始 3月10日 | 内联编辑实际结束为 3月5日 | `Message.error` 提示，编辑取消 |
| DATE-007 | 实际开始晚于计划-红色高亮 | 活动计划开始3月1日，实际开始3月5日 | 查看活动列表 | 实际开始日期文字显示红色 `rgb(var(--danger-6))` |
| DATE-008 | 实际结束晚于计划-红色高亮 | 活动计划结束3月10日，实际结束3月15日 | 查看活动列表 | 实际结束日期文字显示红色 `rgb(var(--danger-6))` |
| DATE-009 | 实际日期未晚于计划-正常色 | 活动计划开始3月5日，实际开始3月3日 | 查看活动列表 | 实际开始日期文字显示默认颜色 |

### 3.3c 进度加权计算

| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| PROG-001 | 工期加权进度-长任务完成 | 活动A工期20天(COMPLETED)，活动B工期5天(NOT_STARTED) | 查看项目进度 | 进度 = (100×20 + 0×5) / 25 = 80% |
| PROG-002 | 工期加权进度-短任务完成 | 活动A工期5天(COMPLETED)，活动B工期20天(NOT_STARTED) | 查看项目进度 | 进度 = (100×5 + 0×20) / 25 = 20% |
| PROG-003 | 无工期fallback权重1 | 活动A无工期(COMPLETED)，活动B无工期(NOT_STARTED) | 查看项目进度 | 进度 = (100×1 + 0×1) / 2 = 50% |
| PROG-004 | 混合有无工期 | 活动A工期10天(IN_PROGRESS)，活动B无工期(COMPLETED) | 查看项目进度 | 进度 = (50×10 + 100×1) / 11 ≈ 54.55% |
