...
| 编号 | 用例名称 | 前置条件 | 操作步骤 | 预期结果 |
|------|---------|---------|---------|---------|
| IMP-001 | 导入 Excel 活动 | 项目存在，有权限 | POST `/api/activities/project/:projectId/import-excel` 上传 xlsx | 200，批量创建活动 |
| IMP-002 | 自动创建联系人 | Excel 含不存在的负责人 | 同上 | 200，自动创建 canLogin=false 用户，createdUsers 列出新建姓名 |
