...
### 12. 表单验证 (form-validation.spec.ts)

| # | 用例 | 预期结果 |
|---|------|----------|
| 12.1 | 空项目表单提交 | 验证错误提示 |
| 12.2 | 缺少项目名称提交 | 名称字段错误 |
| 12.3 | 空活动表单提交 | 验证错误提示 |
| 12.4 | 空产品表单提交 | 验证错误提示 |
| 12.5 | 取消按钮丢弃更改 | 数据不保存 |
...
2. **uniqueName**：所有创建操作使用 `uniqueName()` 生成唯一名称避免冲突
3. **API 验证**：关键操作等待 `waitForResponse` 验证 HTTP 状态码
4. **容错设计**：使用 `.catch(() => false)` 处理可选元素，避免硬性失败
5. **Arco Helper**：所有 Arco Design 组件交互封装在 `helpers/arco.ts`
