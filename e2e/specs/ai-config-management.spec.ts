...
test.describe.serial('AI Config Management @p1', () => {
  ...
  test('create AI config drawer opens with correct fields', async ({ authedPage: page }) => {
    ...
    if (await createBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      const drawer = page.locator('.arco-drawer').filter({ hasText: /新建配置/ });
      if (await drawer.isVisible({ timeout: 5_000 }).catch(() => false)) {
        ...
        const providerSelect = drawer.locator('.arco-select').filter({
        ...
        if (await providerSelect.isVisible({ timeout: 2_000 }).catch(() => false)) {
          ...
          const deepseekOpt = page.locator('.arco-select-popup:visible .arco-select-option').filter({ hasText: 'DeepSeek' });
  ...
  test('empty state or config list displays', async ({ authedPage: page }) => {
    ...
    const hasCard = await page.locator('.arco-card').first().isVisible({ timeout: 3_000 }).catch(() => false);
