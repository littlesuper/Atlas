...
| 10 | notes | 备注 | Input 文本框 | 回车 / 失焦 | 失焦自动提交 |
...
- 无权限时：单元格 cursor 为 default，双击不响应；+ 按钮不显示
- 有权限时：单元格 cursor 为 pointer，双击进入编辑态；+ 按钮可见

## 7. 列设置

### 7.1 新增列自动可见
当系统新增列后，用户已保存的列偏好会自动将新列设为可见。判断逻辑：若列 key 不存在于用户已保存的 `order` 列表中，则视为新增列，自动加入 `visible`。用户手动取消的列不受影响。
