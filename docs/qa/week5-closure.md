...
| ------------------------ | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| Day 1-2 API 集成测试     | 已建立 Atlas 技术栈下的 Vitest + supertest 集成测试入口，并补齐到 23 个明确测试用例 | `server/src/__tests__/integration/auth-projects.integration.test.ts`、`server/src/__tests__/integration/core-api.integration.test.ts`、`package.json` 的 `test:integration` | 已完成                                       |
...
| 验收标准                        | 状态           | 说明                                                                                                                                                                                                           |
| ------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 至少 20 个集成测试              | 已满足         | 当前 `server/src/__tests__/integration/` 下明确的 `it(...)` 用例为 23 个，覆盖认证、权限、用户、项目、活动、检查项、产品、周报和数据库行为。                                                                   |
...
| 类别                 | 文件                                                                                                                                                                                                   |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| API / 数据库集成测试 | `server/src/__tests__/integration/auth-projects.integration.test.ts`、`server/src/__tests__/integration/core-api.integration.test.ts`、`server/src/__tests__/integration/database.integration.test.ts` |
...
| 验证项        | 命令 / 证据                                                            | 结果                                                                     |
| ------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 集成测试数量  | `rg -n "^\\s*(test\|it)\\(" server/src/__tests__/integration \| wc -l` | 23                                                                       |
| 集成测试执行  | `npm run test:integration`                                             | 3 files / 23 tests passed                                                |
...
1. 数据库测试目前使用 SQLite 现实开发环境，没有引入 Testcontainers PostgreSQL 或迁移正反执行验证。
...
Week 5 的数量型验收已经补齐。建议进入 Week 6 可观测性前，保留以下取舍：

1. 暂不引入 Testcontainers PostgreSQL，除非已经准备好在 CI 中承担额外运行时间和数据库服务复杂度。
2. 三浏览器 E2E 继续作为 release validation / targeted smoke 能力，不作为每个 PR 的 required check。
3. 后续扩展视觉和可访问性覆盖时，优先补周报、风险、产品等复杂页面。
