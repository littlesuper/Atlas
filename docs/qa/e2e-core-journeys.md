...
| 3   | Create project from template and verify generated activities                            | Partial: `e2e/specs/template-management.spec.ts` covers template CRUD, `e2e/specs/projects.spec.ts` covers manual project creation                                                                                                                       | Gap     | Add a blocking E2E that creates a template, creates a project from it, and verifies generated activities |
...
| 6   | Activity import/export and undo rollback                                                | Partial: `e2e/specs/activity-export.spec.ts`, `e2e/specs/activity-import.spec.ts`, `e2e/specs/frontend-interactions.spec.ts`                                                                                                                             | Gap     | Add rollback assertions for imported activities and undo stack behavior                                  |
| 7   | Weekly report flow: create from project, save draft, edit content, submit, view summary | `e2e/specs/weekly-report-crud.spec.ts`, `e2e/specs/weekly-report-form.spec.ts`, `e2e/specs/weekly-reports.spec.ts`                                                                                                                                       | Partial | Tighten soft assertions so draft/save/submit failures block CI                                           |
...
1. Add the missing "create project from template" journey.
2. Add a stronger activity import undo rollback journey.
3. Tighten weekly report form tests by replacing non-blocking branches with
   deterministic setup and assertions.
